...
function initAdminPanel() {
    ...
    setupBulkUpload();
    initAddProductForm(); // Initialize the form
}
...
async function addProduct() {
    ...
    console.log('Form values:', { title, price, description, affiliateLink, source, category, image, tags, featured });
...
async function importBulkProducts() {
    ...
    try {
        ...
        saveProducts(products);
        if (document.getElementById('bulk-data')) {
...
function loadAdminDashboard() {
    ...
    const tiktokProductsEl = document.getElementById('tiktok-products');
    const amazonProductsEl = document.getElementById('amazon-products');
    
    if (totalProductsEl) totalProductsEl.textContent = activeProducts.length;
    if (featuredProductsEl) featuredProductsEl.textContent = featuredProducts.length;
    if (tiktokProductsEl) tiktokProductsEl.textContent = tiktokProducts.length;
    if (amazonProductsEl) amazonProductsEl.textContent = amazonProducts.length;
...
function loadRecentActivity() {
    ...
    activityList.innerHTML = recentProducts.map(product => `
        <div class="activity-item" style="display: flex; align-items: center; gap: 1rem; padding: 1rem; border-bottom: 1px solid var(--admin-border);">
            <div class="activity-icon" style="width: 40px; height: 40px; border-radius: 8px; background: var(--admin-accent); display: flex; align-items: center; justify-content: center;">
                <i class="fas fa-box" style="color: white;"></i>
            </div>
            <div class="activity-info">
                <p style="margin: 0; font-weight: 500;"><strong>${product.title}</strong> added</p>
                <span class="activity-time" style="font-size: 0.8rem; color: var(--admin-text-secondary);">${formatPrice(product.price)} • ${product.source}</span>
            </div>
        </div>
    `).join('');
}
